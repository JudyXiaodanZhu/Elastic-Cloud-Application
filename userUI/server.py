...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql://ece1779:secret@172.31.68.23/ece1779'
...
def register():
    ...
    if request.method == 'POST' and form.validate():
        ...
        user = User(form.email.data, hash_var)
        db.session.add(user)
        db.session.commit()
        login_user(user, remember=True)
...
def home():
    ...
    if img:
        for field in img:
            inputs = s3.generate_presigned_url(
...
def upload_file():
    ...
    if request.method == 'POST':
        result = upload_file(request.files, current_user)
        if result[0]:
            return redirect(url_for('upload_file', filename=result[1]))
...
def test():
    ...
    if request.method == 'POST' and form.validate():
        ...
        result = upload_file(request.files, user)
        if result[0]:
            return redirect(url_for('upload_file', filename=result[1]))
        else:
